...
void TextPage::generateUnderlinesAndLinks(GList *columns) {
  ...
  for (colIdx = 0; colIdx < columns->getLength(); ++colIdx) {
	...
	for (wordIdx = 0; wordIdx < line->words->getLength(); ++wordIdx) {
      word = (TextWord *)line->words->get(wordIdx);
      GString* tmpWord = word->getText();
      Unicode lastChar = word->getChar(word->getLength()-1);
      ...
      // separator still stick on word
      int removeLast = 0;
      if(lastChar == 44 && tmpWord->getLength() > 3)
          removeLast = 1;
	    ...
	    if (underline->horiz) {
	      if (word->rot == 0 || word->rot == 2) {
            if (fabs(underline->y0 - base) < ubSlack)
                if(underline->x0 < word->xMin + uSlack)
                    if(word->xMax - uSlack < underline->x1+(removeLast*uSlack)) { // in the case of <u>word</u>, (comma outside of underline)
