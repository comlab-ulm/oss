...
static inline void pr_str(GString *outStr, const char *data) {
    //outStr->append(data, (int)strlen(data));
    outStr->append(data);
}
...
static void pf_str(GString *outStr, const char *fmt, ...) {
  va_list args;
  GString *s;
  va_start(args, fmt);
  s = GString::formatv(fmt, args);
  va_end(args);
  //outStr->append(s->getCString(), s->getLength());
  outStr->append(s->getCString());
  delete s;
}
...
char* HTMLGen::convertPage(
  ...
  pr_str(outStr, "<html>\n");
  pr_str(outStr, "<head>\n");
  pr_str(outStr, "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\">\n");
  pr_str(outStr, "<style type=\"text/css\">\n");
  pr_str(outStr, ".txt { white-space:nowrap; }\n");
  fonts = text->getFonts();
  ...
  for (i = 0; i < fonts->getLength(); ++i) {
    ...
    if (!fontDefn->used && fontDefn->fontFace) {
      pr_str(outStr, fontDefn->fontFace->getCString());
    }
    pf_str(outStr, "#f{0:d} {{ {1:t} }}\n", i, fontDefn->fontSpec);
  ...
  if (primaryDir >= 0) {
    ...
    pr_str(outStr, "<body dir=\"rtl\">\n");
  }
  if (primaryDir >= 0) {
    pf_str(outStr, "<img id=\"background\" style=\"position:absolute; left:0px; top:0px;\" width=\"{0:d}\" height=\"{1:d}\" src=\"{2:s}\">\n",
           (int)(pageW * zoom), (int)(pageH * zoom), pngURL);
  } else {
    pf_str(outStr, "<img id=\"background\" style=\"position:absolute; right:0px; top:0px;\" width=\"{0:d}\" height=\"{1:d}\" src=\"{2:s}\">\n",
           (int)(pageW * zoom), (int)(pageH * zoom), pngURL);
  }
  ...
  for (colIdx = 0; colIdx < cols->getLength(); ++colIdx) {
	...
	s = new GString();
    for (firstWordIdx = (primaryDir >= 0) ? 0 : words->getLength() - 1; (primaryDir >= 0) ? firstWordIdx < words->getLength(): firstWordIdx >= 0;firstWordIdx = lastWordIdx + primaryDir) {
	...
	if (primaryDir >= 0) {
      pf_str(outStr, "<div class=\"txt\" style=\"position:absolute; left:{0:d}px; top:{1:d}px;\">{2:t}</div>\n",
             (int)(line->getXMin() * zoom),
             (int)(line->getYMin() * zoom), s);
	} else {
      pf_str(outStr, "<div class=\"txt\" style=\"position:absolute; right:{0:d}px; top:{1:d}px;\">{2:t}</div>\n",
...
//  printf("%s", outStr->getCString());
//  delete outStr;
  return outStr->getCString();
//  return errNone;
}
...
void HTMLGen::appendSpans(GList *words, int firstWordIdx, int lastWordIdx,
  ...
  for (wordIdx = firstWordIdx;
    ...
    GString * tmpWord;
    for(int _i = 0; _i < word1->getLength()-1; _i++){
        tmpWord = word1->getText();
    }

    if(word1->isUnderlined())
        s->append("<u>");
