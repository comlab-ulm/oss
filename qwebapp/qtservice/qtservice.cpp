...
int QtServiceBase::exec()
{
    if (d_ptr->args.size() > 1) {
        ...
        if (a == QLatin1String("-i") || a == QLatin1String("-install")) {
            ...
            printf("Version: ");
            printf(QString(QString::number(MaN)+"."+QString::number(MiN)+"."+QString::number(VER - MINUS)).toStdString().c_str());
            printf("\n");
            printf("The service\n"
            ...
            return ec;
        } else if (a == QLatin1String("-m") || a == QLatin1String("-migrade-db")) {
            int ec = d_ptr->run(false, d_ptr->args);
            if (ec == -1)
                qErrnoWarning("The service could not be executed.");
            return ec;
            ...
            return 0;
        } else if (a == QLatin1String("-d") || a == QLatin1String("-delete")) {
            int ec = d_ptr->run(false, d_ptr->args);
            if (ec == -1)
                qErrnoWarning("The service could not be executed.");
            return ec;
        } else if (a == QLatin1String("-j") || a == QLatin1String("-json")) {
            if (d_ptr->args.size() > 2) {
                int ec = d_ptr->run(false, d_ptr->args);
                if (ec == -1)
                    qErrnoWarning("The service could not be executed.");
            ...
            printf("\n%s -[i|u|e|t|d|j|p|r|c|v|h]\n"
                   ...
                   "\t-j(son) string\t\t: Add a local JSON open config file to the license file.\n"
                   "\t-d(elete)\t\t\t: Delete all local changes from the license file.\n"
                   "\t-p(ause)\t: Pause the service.\n"
                   "\t-r(esume)\t: Resume a paused service.\n"
                   "\t-m(igrade-db)\t: Update DB structure to the last version.\n"
