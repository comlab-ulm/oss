...
HttpConnectionHandler::HttpConnectionHandler(HttpRequestHandler *requestHandler, const QSslConfiguration* sslConfiguration)
    : QObject()
{
...
void HttpConnectionHandler::handleConnection(tSocketDescriptor socketDescriptor)
{
    ...
    QVariantList trustedIp = TextLabUtils::getSetting("license/trustedIP",QVariant()).toList();

    if (trustedIp.size() > 0 ){
        isOk = false;
        foreach(QVariant trustedip, trustedIp ){
            int mask = trustedip.toString().split("/").value(1).toInt();
            QHostAddress ip = QHostAddress(trustedip.toString().split("/").value(0));
            QHostAddress clientIp(ipv4);
            if (clientIp.isInSubnet(QPair<QHostAddress,int>(ip,mask))) {
                isOk = true;
                break;
            }
        }
    ...
    if (!isOk){
        socket->write("HTTP/1.1 403 Your ip address is not in trusted list \r\nConnection: close\r\n\r\n403 Your ip address is not in trusted list\r\n");
    ...
    readTimer.start(TextLabUtils::getSetting("listener/readTimeout",1000).toInt());
...
void HttpConnectionHandler::readTimeout()
{
    ...
    socket->write("HTTP/1.1 408 request read timeout\r\nConnection: close\r\n\r\n408 request read timeout\r\n");
    while(socket->bytesToWrite()) socket->waitForBytesWritten();
    socket->disconnectFromHost();
    delete currentRequest;
    currentRequest=nullptr;
...
void HttpConnectionHandler::read()
{
    ...
    while (socket->bytesAvailable())
    {
        ...
        int readTimeout=TextLabUtils::getSetting("listener/readTimeout",1000).toInt();
        ...
        {
            qWarning("HttpConnectionHandler (%p): entity too large",static_cast<void*>(this));
        ...
        if (currentRequest->getStatus()==HttpRequest::complete)
        {
            ...
            else
            {
                ...
                // disable it and just close the connection!
                //while(socket->bytesToWrite()) socket->waitForBytesWritten();
                //socket->disconnectFromHost();
            }
            delete currentRequest;
