...
void FileLogger::refreshSettings()
{
    ...
    fileName=TextLabUtils::getSetting("logging/fileName").toString();
    ...
    maxSize=TextLabUtils::getSetting("logging/maxSize",0).toLongLong();
    maxBackups=TextLabUtils::getSetting("logging/maxBackups",0).toInt();
    msgFormat=TextLabUtils::getSetting("logging/msgFormat","{timestamp} {type} {msg}").toString();
    timestampFormat=TextLabUtils::getSetting("logging/timestampFormat","yyyy-MM-dd hh:mm:ss.zzz").toString();
    minLevel=static_cast<QtMsgType>(TextLabUtils::getSetting("logging/minLevel",0).toInt());
    bufferSize=TextLabUtils::getSetting("logging/bufferSize",0).toInt();
...
FileLogger::FileLogger(const int refreshInterval, QObject* parent)
    : Logger(parent)
{
